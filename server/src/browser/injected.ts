...
(function(){
  ...
  function frames(buf){var out=[];while(true){var nl=buf.indexOf('\\n');if(nl<0)break;var sz=parseInt(buf.substring(0,nl),10);if(isNaN(sz))break;if(buf.length<nl+1+sz)break;out.push(buf.substring(nl+1,nl+1+sz));buf=buf.substring(nl+1+sz);if(buf.charAt(0)==='\\n')buf=buf.substring(1);}return{buf:buf,out:out};}
