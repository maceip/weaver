...
class TaskTrackerTest {
    ...
    fun setUp() {
        ...
        context.getSharedPreferences("weaver_tasks", Context.MODE_PRIVATE)
            .edit().clear().commit()
    }
    ...
    fun submitOnlineIsRunning() {
        ...
        assertEquals(TaskStatus.Running, tracker.tasks.value.single().status)
    }
    ...
    fun settleOldestRunningCanFail() {
        ...
        assertEquals(TaskStatus.Failed, tracker.tasks.value.single().status)
    }
    ...
    fun settleWithNoRunningTaskIsNoop() {
        ...
        tracker.settleOldestRunning()
        assertEquals(TaskStatus.Queued, tracker.tasks.value.single().status)
    }
    ...
    fun tasksSurviveANewInstance() {
        ...
        assertEquals(TaskStatus.Queued, reloaded.tasks.value.single().status)
        assertEquals("queued offline", reloaded.tasks.value.single().prompt)
    }
}
