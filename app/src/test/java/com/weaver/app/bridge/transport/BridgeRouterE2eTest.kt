...
class BridgeRouterE2eTest {
    ...
    @Test
    fun localAuthed_thenRemoteComesOnline_staysLocal() = runTest {
    ...
    @Test
    fun remoteOnly_localNeverAuthed_routesRemote() = runTest {
        val local = FakeTransport("local")
        val remote = FakeTransport("remote")
        val router = routerOf(local, remote, backgroundScope, { testScheduler.currentTime })
        runCurrent()
    ...
    @Test
    fun remoteFirst_thenCookiesShared_transitionsToLocal() = runTest {
        val local = FakeTransport("local")
        val remote = FakeTransport("remote")
        val router = routerOf(local, remote, backgroundScope, { testScheduler.currentTime })
        runCurrent()
    ...
    @Test
    fun remoteFirst_thenUserLogsIntoLocalWebview_transitionsToLocal() = runTest {
        val local = FakeTransport("local")
        val remote = FakeTransport("remote")
        val router = routerOf(local, remote, backgroundScope, { testScheduler.currentTime })
        runCurrent()
    ...
    @Test
    fun localFails_failsOverToRemote() = runTest {
        val local = FakeTransport("local")
        val remote = FakeTransport("remote")
        val router = routerOf(local, remote, backgroundScope, { testScheduler.currentTime })
        runCurrent()
    ...
    @Test
    fun flappingRemote_isBenchedByBreaker() = runTest {
        val local = FakeTransport("local")
        val remote = FakeTransport("remote")
    ...
    @Test
    fun outboundFromEitherBackendReachesTheSink() = runTest {
        val local = FakeTransport("local")
        val remote = FakeTransport("remote")
        val router = routerOf(local, remote, backgroundScope, { testScheduler.currentTime })
    ...
    @Test
    fun twoPhones_routersAreIndependent() = runTest {
        ...
        val aLocal = FakeTransport("local"); val aRemote = FakeTransport("remote")
        val bLocal = FakeTransport("local"); val bRemote = FakeTransport("remote")
    ...
    @Test
    fun twoUsersShareRemote_thenDivergeToOwnLocalWebviews() = runTest {
        ...
        val u1Local = FakeTransport("local"); val u1Remote = FakeTransport("remote")
        val u2Local = FakeTransport("local"); val u2Remote = FakeTransport("remote")
